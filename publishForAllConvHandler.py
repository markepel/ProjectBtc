...
def publishForAll(bot, update):
  global forAll_state
  logger.info('Initial forAll_state - {0} in publishForAll for chat_id {1}'.format(str(forAll_state), str(update.message.chat_id)))
  logger.info('PublishForAll Starts for chat_id = {0}'.format(str(update.message.chat_id)))
...
def text(bot, update):
  global forAll_state
  forAll_state[str(update.message.chat_id)] = update.message.text
  logger.info('PublishForAll text = {0}'.format(update.message.text))
  logger.info('forAll_state - {0} for text in publishForAll for chat_id {1}'.format(str(forAll_state), str(update.message.chat_id)))
...
def finish(bot, update):
  try:
    db = DBRepo()
    global forAll_state
    logger.info('forAll_state - {0} on finish in publishForAll for chat_id {1}'.format(str(forAll_state), str(update.message.chat_id)))
    idsToPublish = db.get_all_users_ids()
    logger.info('PublishForAll ids = {0}'.format(str(idsToPublish)))
    for id in idsToPublish:
      id = id[0]
      try:
        bot.send_message(chat_id=id, text=forAll_state[str(update.message.chat_id)], reply_markup=ReplyKeyboardMarkup(reply_keyboard_main_menu, one_time_keyboard=True), parse_mode=telegram.ParseMode.HTML)
        time.sleep(0.03)
      except Exception as e:
        logger.info('Bad request exception on send to all - ', e)
        db.delete_user(id)

    logger.info('PublishForAll finished for chat_id {0} successfully'.format(str(update.message.chat_id)))
    del forAll_state[str(update.message.chat_id)]
    logger.info('forAll_state - {0} after finish in publishForAll for chat_id {1}'.format(str(forAll_state), str(update.message.chat_id)))
...
def cancel(bot, update):
  global forAll_state
  logger.info('forAll_state - {0} on cancel in publishForAll for chat_id {1}'.format(str(forAll_state), str(update.message.chat_id)))
  ...
  if str(update.message.chat_id) in forAll_state:
    del forAll_state[str(update.message.chat_id)]
  logger.info('forAll_state - {0} after cancel in publishForAll for chat_id {1}'.format(str(forAll_state), str(update.message.chat_id)))
  return ConversationHandler.END
