...
class DBRepo:
  ...
  def add_strategy(self, name, description, price, rightawayLink, dateOfCreation):
    command = "INSERT INTO strategies (name , description, price, rightaway_link, date_of_creation) VALUES (?, ?, ?, ?, ?)"
    args = (name, description, price, rightawayLink, dateOfCreation)
  ...
  def add_subscription_for_strategy(self, userId, strategyId, receivingTime, dateOfPurchase):
    ...
    args = (userId, strategyId, receivingTime, 1,  dateOfPurchase)
    res =self.conn.execute(stmt, args)
    self.conn.commit()
  ...
  def setup(self):
    ...
    dropStrategiesTableCommand = "DROP TABLE IF EXISTS strategies"
    ...
    dropSubscriptionsTableCommand = "DROP TABLE IF EXISTS subscriptions"
    ...
    self.conn.execute(dropStrategiesTableCommand)

    self.conn.execute(createStrategiesTableCommand)

    self.conn.execute(dropSubscriptionsTableCommand)
