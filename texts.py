...
Данный бот
 ...
 – Ваш личный путеводитель в мире криптовалют. Он поможет Вам научиться торговать на криптовалютных рынках, а самое главное – зарабатывать! Здесь вы найдете:
     ...
     <b>Стратегии</b> - это свод правил и действий при определенных ситуациях на рынке. Мы расскажем вам суть стратегии, объясним как это работает на практике и в течение месяца будем определять для вас точки входа и выхода из трейда. Выполняя наши рекомендации, вы наглядно убедитесь как можно зарабатывать, применяя торговые системы.
     ...
     <b>📂 Материалы</b> – в этом разделе вы найдете интересные книги и видео, зарекомендовавшие себя по всему миру, описывающие финансовые рынки, принципы их работы и способы заработка.""".format(name)
...
<a href="https://www.youtube.com/watch?v=rAoz5LMGH0M&t=38s">Ставка на Bitcoin</a>
  ...
  def getTextForSubscriptionForSignals():
    return """🎉Поздравляем! Вы приобрели доступ к сигналам. В скором времени Вам придет первый сигнал! А сейчас  рекомендуем ознакомиться со статьей о риск-менеджменте и инструкцией по использованию сигналов. Успешной торговли🤝."""

  @staticmethod
  def getTextForSubscriptionForStrategy(strategyName):
    return """🎉Поздравляем! Вы приобрели стратегию. Наличие стратегии отобразится в вашем личном кабинете. А сейчас вы можете ознакомиться с обучающими материалами, статьей о риск-менеджменте и инструкцией по использованию стратегии. Успешной торговли🤝.""".format(strategyName)
  ...
  def getTextForProfile(strategyNamesAndExpireDatesDict, isSubscribedOnSignalsAndExpireDateDict):
    ...
    if not strategyNamesAndExpireDatesDict:
      strategiesText += "Нет изучаемых приобретенных стратегий"
    else:
    ...
    if not isSubscribedOnSignalsAndExpireDateDict:
      signalsInfo = "Подписка на сигналы не приобретена"
    else:
  ...
  def getTextForStrategies():
    return """
☝️Возможно, вы уже знаете, что дисциплина – самое важное в трейдинге! Чтобы соблюдать
ее, необходимо придерживаться стратегии. Стратегия – это свод правил и действий
при определенных ситуациях на рынке.

Молодой криптовалютный рынок позволяет нам применять многие авторские торговые
системы проверенные временем. Мы собрали команду первоклассных трейдеров для
составления и внедрения стратегий в массы.👨🏻‍💻

Наша задача – помочь вам чувствовать рынок. Мы расскажем суть стратегии,
объясним как это работает на практике и в течение месяца будем определять для вас
точки входа и выхода из трейда. Каждый рабочий день мы будем присылать: 

1. График интересующей нас криптовалюты с построенными индикаторами/фигурами/инструментами используемыми в выбранной Вами стратегии. 📈
2. Точки покупки/продажи по этому графику. 🔼🔽🕰
3. Пояснение к действиям (Почему именно эта криптовалюта; Как были построены индикаторы/фигуры/инструменты; Дальнейшее развитие событий.)❔
 
Следуя нашим рекомендациям, Вы наглядно убедитесь как работают торговые системы.

🎓Стоимость стратегии это вложение в будущее. Перед вами открывается не
только возможность обучаться, но и возможность зарабатывать во время обучения,
разумеется, если научитесь учитывать и анализировать входные параметры и оценивать риски.(Бонусом к стратегии вы получите мини-курс по риск-менеджменту)

У вас будет 5 стратегий на выбор. Вы можете как проходить все по очереди, так и
приобрести сразу несколько торговых идей. Всё зависит от желания и трудоспособности.
Каждый месяц мы будем добавлять новую стратегию, увеличивая уровень сложности,
...
У каждой стратегии есть свои показатели/свойства. 📊 Если хотите закрывать сделку в течении нескольких дней, то выбирайте «Время закрытия сделки: Краткосрочная», если хотите торговать на бирже Bittrex, смотрите, чтоб напротив показателя Биржа
была площадка Bittrex, Если хотите получать сигналы часто- напротив показателя
«Количество сигналов» должна быть минимум цифра 20. Выбирайте любую стратегию и
...
"""
  ...
  def getTextForSignals():
    return """
💰Сигналы:
В данном разделе вы получите рекомендации по торговле и эксклюзивные инсайты от трейдеров нашей команды. Не затрачивая время на длительное изучение и анализ рынка вы сможете уже на следующий день после приобретения подписки совершать прибыльные сделки, опираясь на наши рекомендации. Каждый день с помощью фундаментального и технического анализа мы выявляем перспективные монеты, отбираем одну или две криптовалюты, которые, по нашему мнению, вырастут в цене в ближайшие 2-4 дня и публикуем сообщения с названием монет. 
